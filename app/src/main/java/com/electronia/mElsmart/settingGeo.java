...
public class settingGeo extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_setting_geo);
    ...
    public Integer InsertSysSetting()
    {
        ...
        try {
            ...
            database.execSQL("INSERT INTO system_setting(AutoPunchGeo)VALUES("+AutoPunch+")" );
            res=1;
        }
        catch (Exception ex) {
            res=0;
    ...
    public Integer UpdateSysSetting()
    {
        ...
        try {
            database = db.getWritableDatabase();
            database.execSQL("UPDATE system_setting set AutoPunchGeo=0");// (date,BadgeNo,Name,Ter,direction,empid)VALUES('"+txtdatetime.getText()+"','"+badgeno+"','"+name+"' ,'"+compara.termo+"','"+s+"' ,"+empID+")" );
            res=1;
        }
        catch (Exception ex) {
            res=0;
    ...
    public Integer UpdateAutoPunch(Integer Autopunch)
    {
        ...
        try {
            database = db.getWritableDatabase();
            database.execSQL("UPDATE system_setting set AutoPunchGeo= "+Autopunch);// (date,BadgeNo,Name,Ter,direction,empid)VALUES('"+txtdatetime.getText()+"','"+badgeno+"','"+name+"' ,'"+compara.termo+"','"+s+"' ,"+empID+")" );
            res=1;
        }
        catch (Exception ex) {
            res=0;
    ...
    private void PAlertDialog(String title, String msg)
    {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(settingGeo.this);
